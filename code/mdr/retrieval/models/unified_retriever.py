...
class UnifiedRetriever(nn.Module):
    ...
    def __init__(self,
        ...
        if "roberta" in args.model_name:
            ...
            self.roberta = False
        self.stop = nn.Linear(config.hidden_size, 2)  #TJH 2 outputs because using CE in unified_loss, not BCE..
        self.stop_drop = nn.Dropout(args.stop_drop)

    def encode_seq(self, input_ids, mask, type_ids):
        if self.roberta:
            cls_rep = self.encoder(input_ids, mask)[0][:, 0, :]  #TJH [bs, max_seq_len, hs] TODO why encoder and not encoder_c?

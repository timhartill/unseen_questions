...
outfile = os.path.join(OUT_DIR, 'tmp_numeric_vs_nonnumeric_answers_3runs_no_train_overlap.txt')
...
unifiedqa_base_train_orig_tdnd = ["narrativeqa", "ai2_science_middle", "ai2_science_elementary",
                             "arc_hard", "arc_easy", "mctest_corrected_the_separator",
                             "squad1_1", "squad2", "boolq", "race_string", "openbookqa", 
                             "synthetic_numeric", "synthetic_textual"]
...
def calc_answer_set(s, dsetset, ngram='Unigram'):
    ...
    for dset in dsetset:     # for each eval dataset in eval_datasets_memo
        ...
        out_dict[dset] ={'gt':[],  # list of answers
                         'gt_norm':[],  # list of normalised answers strips the, a, an
                         'predictions': [],  # pred answer
                         'predictions_norm': [],  # normalised pred answer
                         'sim_scores': [],
                         'sim_buckets': [],
                         'pred_scores': [],
                         'gt_norm_unique': {},
                         'gt_norm_unique_count': 0,
                         'numeric_idx': [],  # list of indices of numeric answers 
                         'non_numeric_idx': [],
                         'is_numeric': [],
                         'count_all_dedup_nonnumeric': 0,  # already dedup, remove numeric here
                         'count_leastsim': 0,  # remove >= 60
                         'count_leastsim_ans_not_in_train': 0  # filled in later
                        }
        for ind in range(orig_num):
            ...
            answer_norm = eval_metrics.normalize_answer_squad(answer)  # Lower text and remove punctuation, articles and extra whitespace.
            out_dict[dset]['gt_norm'].append(answer_norm)
            if dset in eval_metrics.textual_answers_only and answer_norm.isdecimal():  # drop, drop-cs
                out_dict[dset]['numeric_idx'].append(ind)
                out_dict[dset]['is_numeric'].append(True)
            else:
                out_dict[dset]['non_numeric_idx'].append(ind)
                out_dict[dset]['is_numeric'].append(False)
                out_dict[dset]['count_all_dedup_nonnumeric'] += 1
                
            pred_norm = eval_metrics.normalize_answer_squad(pred_answer)
            ...
            if combo_score < 60:
                bucket = '0:60'
                if not out_dict[dset]['is_numeric'][-1]:
                    out_dict[dset]['count_leastsim'] += 1    
            elif combo_score < 90:
                bucket = '60:90'
            else:
                bucket = '90:101'
            out_dict[dset]['sim_buckets'].append(bucket)  # leat sim = 0:60 and not numeric
        ...
        print('#'*25)
        print(f"DATASET: {dset} ALL Dedup NonNumeric: {out_dict[dset]['count_all_dedup_nonnumeric']}  LEAST SIM: {out_dict[dset]['count_leastsim']}")
        print('#'*25)
...
def add_ans_not_in_train_count(orig_tdnd, dsetset, train_answers):
    print('Splitting each train answer....')
    train_answers_split = [set(t.split()) for t in train_answers]
    for dset in dsetset:
        print(f"Processing {dset}...")
        orig_tdnd[dset]['ans_in_train'] = []
        for ind, gt_norm in enumerate(orig_tdnd[dset]['gt_norm']):
            if orig_tdnd[dset]['sim_buckets'][ind] == '0:60' and not orig_tdnd[dset]['is_numeric'][ind]: 
                if gt_norm in train_answers:  # check exact lookup first for speed
                    orig_tdnd[dset]['ans_in_train'].append(True)
                else:
                    gt_norm_split = set(gt_norm.split())
                    found = False
                    for train_norm_split in train_answers_split:
                        if gt_norm_split.intersection(train_norm_split):
                            found = True
                            orig_tdnd[dset]['ans_in_train'].append(True)
                            break
                    if not found:
                        orig_tdnd[dset]['count_leastsim_ans_not_in_train'] += 1
                        orig_tdnd[dset]['ans_in_train'].append(False)
        print(f"Dataset: {dset} leastsim not in train: {orig_tdnd[dset]['count_leastsim_ans_not_in_train']}")
    return
...
train_answers = set()
for dset in unifiedqa_base_train_orig_tdnd:
    trainfile = os.path.join(DATASET_DIR, dset, 'train.tsv')
    train = utils.load_uqa_supervised(trainfile, ans_lower=False, verbose=True, return_parsed=True) 
    this_dset_answers = [eval_metrics.normalize_answer_squad(s['answer']) for s in train if not eval_metrics.normalize_answer_squad(s['answer']).isdecimal() and not eval_metrics.normalize_answer_squad(s['answer'])=='']
    train_answers = train_answers.union(this_dset_answers)
print(f"Number of unique answers excluding numbers across uqa+tdnd datasets: {len(train_answers)}")  #398915
...
# bucket test-train similarity vs results on the uqa+tdnd model
s_tdnd_summary = SimilarityAggregator(sim_results, no_overlap_thresh=1000.0, results_list=results_list_tdnd,
                                      compare_over=unifiedqa_base_train_orig_tdnd,
                                      thresh_buckets = [0,60,90,101])

orig_tdnd = calc_answer_set(s_tdnd_summary, dsetset=eval_datasets_memo)

add_ans_not_in_train_count(orig_tdnd, eval_datasets_memo, train_answers)
...
Splitting each train answer....
Processing drop_dedup...
Dataset: drop_dedup leastsim not in train: 101
Processing contrast_sets_drop_dedup...
Dataset: contrast_sets_drop_dedup leastsim not in train: 13
Processing physical_iqa...
Dataset: physical_iqa leastsim not in train: 3
Processing commonsenseqa...
Dataset: commonsenseqa leastsim not in train: 19
Processing qasc...
Dataset: qasc leastsim not in train: 21
Processing ropes...
Dataset: ropes leastsim not in train: 62
Processing newsqa...
Dataset: newsqa leastsim not in train: 93

calculating for result set: unifiedqa_bart_large_v7indiv_digits_tdnd
#########################
DATASET: drop_dedup ALL Dedup NonNumeric: 3277  LEAST SIM: 867
#########################
Processing contrast_sets_drop_dedup ...
#########################
DATASET: contrast_sets_drop_dedup ALL Dedup NonNumeric: 478  LEAST SIM: 154
#########################
Processing physical_iqa ...
#########################
DATASET: physical_iqa ALL Dedup NonNumeric: 1838  LEAST SIM: 1354
#########################
Processing commonsenseqa ...
#########################
DATASET: commonsenseqa ALL Dedup NonNumeric: 1221  LEAST SIM: 233
#########################
Processing qasc ...
#########################
DATASET: qasc ALL Dedup NonNumeric: 926  LEAST SIM: 139
#########################
Processing ropes ...
#########################
DATASET: ropes ALL Dedup NonNumeric: 1688  LEAST SIM: 307
#########################
Processing newsqa ...
#########################
DATASET: newsqa ALL Dedup NonNumeric: 4341  LEAST SIM: 1204
#########################
Finished.
...
Dataset: UQA+TDND drop_dedup:  #drop_dedup SIM TO UQA+TDND model
Numeric to Non-Numeric Answer Ratio: 1.67 Numeric Count: 5457 Non-Numeric Count: 3277

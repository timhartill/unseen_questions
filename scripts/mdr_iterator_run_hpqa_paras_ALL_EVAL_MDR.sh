# Run iterator ie retrieve/stage1/stage2 on previously trained models/encoded corpus USING ORIGINAL MDR CKPT
# Will build hnsw index if this option selected and index doesnt exist

# BULK VERSION TO BUILD CONTEXT FOR UNSEEN EVAL DATASETS FROM ORIGINAL MDR CKPT
...
echo "BUILD CONTEXT FOR MUSQUE MU DEV INITIAL CONTEXT..."
python mdr_searchers.py \
    --prefix ITER_fullwiki_mudev_test81ic_b150_h4_mdr_orig \
    --output_dir $LDATA/out/mdr/logs \
    --output_dataset $UQA_DIR/musique_mu_dev_inital_contextv2_fullwiki_bs150_mdr/dev.tsv \
